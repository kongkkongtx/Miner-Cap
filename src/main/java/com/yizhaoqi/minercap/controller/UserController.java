...
public class  UserController {
    ...
    public ResponseEntity<?> register(@RequestBody UserRequest request) {
        LogUtils.PerformanceMonitor monitor = LogUtils.startPerformanceMonitor("USER_REGISTER"); //启动性能监控器实例，用于监控特定操作的执行性能
        try {
    ...
    public ResponseEntity<?> login(@RequestBody UserRequest request) {
        LogUtils.PerformanceMonitor monitor = LogUtils.startPerformanceMonitor("USER_LOGIN");//启动性能监控器实例，用于监控特定操作的执行性能
        try {
            //如果参数为空，则报错
            if (request.username() == null || request.username().isEmpty() ||
                    request.password() == null || request.password().isEmpty()) {
            ...
            String username = userService.authenticateUser(request.username(), request.password());//验证用户身份
            if (username == null) {
    ...
    public ResponseEntity<?> getCurrentUser(@RequestHeader("Authorization") String token) {
        ...
        // 从请求头中获取token
        try {
            //先从Authorization请求头信息,将请求头中的Bearer去除获得token
